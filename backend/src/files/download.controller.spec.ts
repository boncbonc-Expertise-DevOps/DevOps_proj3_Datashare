...
describe('DownloadController (routes /download)', () => {
  const fileService = {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    controller = new DownloadController(fileService);
  });

  describe('GET /download/:token/meta', () => {
  ...
  describe('GET /download/:token', () => {
    ...
    it('sets headers and returns StreamableFile', async () => {
      const fp = 'C:/tmp/dl-test.txt';
      fileService.preparePublicDownload.mockResolvedValueOnce({
        streamPath: fp,
        mimeType: 'text/plain',
        sizeBytes: 5,
        contentDisposition: 'attachment; filename="dl-test.txt"',
      });
